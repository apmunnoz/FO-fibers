...
# Functional setting for T, Phi, phi
...
drhophisfdt = rhof * idt * (sf*phi - sf_n*phi)
FT = getNonlinearHeatForm(T, qT, dSsdt, drhophisfdt, p, sf, Vf, KQ, rhof, source=source)
# Add Robin coefficients
FT += hb * (T - T0) * (1-index) * qT * ds(T_robin) + he * (T - Tsaline) * index * qT * ds(T_robin)

file = VTKFile(f"output/heat-catheter-{dim}D.pvd")
